...
def get_weather(
    ...
    log.info(
        f"Requested latitude: {latitude} and longitude: {longitude} with {options}"
    )
...
def html_output(
    ...
    values = data["hourly"]["temperature_2m"][:limit]
    values2 = data["hourly"]["rain"][:limit]
    labels = ",".join(labels)
    values = str(values)[1:-1]
    values2 = str(values2)[1:-1]
    return templates.TemplateResponse(
        "weather.html",
        {
            "request": request,
            "data": [data, labels, values, values2],
...
async def weather_send(
    ...
    return html_output(request=request, options=options, city=city.title())
